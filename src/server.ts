...
passport.use(new LocalStrategy(
    ...
    (username, password, done) => {
        ...
        axios.get(`http://localhost:8000/users?username=${username}`) //here we are making a get request to our users api to get the user with the email that was sent in the request body 
            .then((res: { data: any[]; }) => {
                const user = res.data[0] //here we are getting the user from the response data
                if (!user) {
                    return done(null, false, { message: 'Invalid credentials.\n' });
                }
                if (!bcrypt.compareSync(password, user.password)) { //here we are comparing the password that was sent in the request body with the hashed password of the user that we got from the database
                    return done(null, false, { message: 'Invalid credentials.\n' });
                }
...
passport.serializeUser((user:any, done) => {
    ...
    console.log('inside local serializeUser. User id is save to the session file store here');
...
passport.deserializeUser((id, done) => {
    ...
    axios.get(`http://localhost:8000/users/${id}`) //here we are making a get request to our users api to get the user with the id that was saved in the session
